...
sha256sums=('c4fb0fe2b8d2d3bd5e72763e80a8ae05b7d44dbac8f8e3bb18ef0161c7266926'
            'd8ddb0385fa1dbf0599b345cbb0655cf605f43668234884cf892d44cb2f04320'
            ...
            '433cc79cffe3266660924c09be3d8209127065eba1740cb9e22bee54154c6768'
...
prepare() {
  ...
  sed -i '1,44d' "$srcdir/$(basename ${_patches[5]})" 
  sed -i 's/size_t colornamelen/unsigned int tabspaces/g' "$srcdir/$(basename ${_patches[5]})"
  sed -i '30,44d' "$srcdir/$(basename ${_patches[5]})" 
  sed -i '1,68d' "$srcdir/$(basename ${_patches[7]})" 
